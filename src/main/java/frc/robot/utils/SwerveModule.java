...
public class SwerveModule {
  ...
  public SwerveModule(S_MODULE_DETAILS module_details, ShuffleTabController shuffleTab) {
    ...
    //--------------INIT--------------
    // create loggger
    logger = new Logger("swerve-" + module_details.CAN_ID_DRIVE, new String[] { "Pos", "Vel", "Ang", "Ang Rate", "Target Vel", "Target Ang" });

    // create shuffle tab
    this.shuffleTab = shuffleTab;
    shuffleTab.createWidget("Drive " + module_details.CAN_ID_DRIVE, BuiltInWidgets.kNumberBar, 0 + (((module_details.CAN_ID_DRIVE-1) % 2) * 3), 0 + (((module_details.CAN_ID_DRIVE-1) / 2) * 2), 1, 2)
      .withProperties(Map.of("Min", -10, "Max", 10));
    shuffleTab.createWidget("Turn " + module_details.CAN_ID_STEER, BuiltInWidgets.kGyro, 1 + (((module_details.CAN_ID_DRIVE-1) % 2) * 3), 0 + (((module_details.CAN_ID_DRIVE-1) / 2) * 2), 2, 2)
    ...
    //--------------STEER MOTOR--------------
    turnMotor = new CANSparkMax(module_details.CAN_ID_STEER, MotorType.kBrushless);
    ...
    // turnEncoder.setZeroOffset(angularOffset); do not use (clears manual encoder zero)
    ...
    //turnController.setSmartMotionAccelStrategy(AccelStrategy.kSCurve, 0);                                 //TODO test tune
    //turnController.setSmartMotionMaxAccel(SwerveDriveConstants.MAX_ANGULAR_ACCELERATION, 0);
  ...
  public SwerveModuleState getDesiredState(boolean optimizedAngle, boolean moduleRel) {
    ...
    state = moduleRel ? state : new SwerveModuleState(state.speedMetersPerSecond, 
  ...
  public Rotation2d getRotation2d(boolean optimizedAngle) {
    ...
    if(Robot.isSimulation()){angle = desiredState.angle;}
